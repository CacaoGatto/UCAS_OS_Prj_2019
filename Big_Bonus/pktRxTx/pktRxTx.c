...
*/
unsigned char dest_mac_addr[6] = { 0x80, 0xfa, 0x5b, 0x33, 0x56, 0xef };
unsigned char src_mac_addr[6] = { 0x00, 0x0e, 0xc6, 0xd7, 0x77, 0x7b };
...
send_packet(char *str, int len)
{
    ...
    //else a=0;
	//else if(len!=6) a=2;
	memcpy((pkt + PAYLOAD_OFFSET + 1), &a, 1);
	memcpy((pkt + PAYLOAD_OFFSET + 2), str, len);
...
*/
    ...
    if (mode == LISTENING_MODE) {
        ...
        printf("\nPress any key to send or 'quit' for exit\n");
        printf("> ");
        //while (1) {
            //ret = printf("> ");
            ret = scanf("%s", cmd);
            ...
            ret = printf("> ");
            FILE *plane = fopen("plane","r+");
            ret = fseek(plane,0L,SEEK_END);
            int flen = ftell(plane);
            ret = printf("flen = %d\n",flen);
            
            //ret = printf("> ");
            char *p = (char *) malloc(flen+1);
			
            ret = fseek(plane, 0L, SEEK_SET);
            ret = fread(p,sizeof(char), flen, plane);
            Elf32_Ehdr eh;
            Elf32_Phdr ph;
            memcpy( &eh, p, sizeof(eh) );
            uint32_t e_phoff = (uint32_t ) eh.e_phoff;
            memcpy(&ph,p+e_phoff,sizeof(Elf32_Phdr));

            uint32_t p_filesz = ph.p_filesz;
            uint32_t p_memsz  = ph.p_memsz;
            uint32_t p_offset = ph.p_offset;
            ret = printf("e_phoff=0x%x, filesz=0x%x, offset=0x%x \n", e_phoff, p_filesz, p_offset);
            uint32_t *tp = (uint32_t *) (p + p_offset);
            uint32_t left_len = p_filesz + p_offset;
            ...
            for (count = 0; count < 24; count++) {
                ...
                ret = fread(p,sizeof(char), part, plane);
                send_packet(p, part);
                ret = printf("> package: %d. length: %d\n", count, part);
                time_wait(5);
            }
            ...
            ret = fread(p,sizeof(char), flen, plane);
            send_packet(p, flen);
            ret = printf("> package: %d\n", ++count);
            time_wait(5);
            /*
            ret = fseek(plane, 0L, SEEK_SET);
            ret = fread(p,sizeof(char), 524, plane);
            send_packet(p, 524);
            ret = printf("> package: %d\n", ++count);
            time_wait(5);
            /*
            while (num--) {
                ret = fseek(plane, 0L, SEEK_SET);
                ret = fread(p,sizeof(char), 900, plane);
                send_packet(p, 900);
                ret = printf("package: %d\n", count++);
                time_wait(5);

                ret = fread(p,sizeof(char), 910, plane);
                send_packet(p, 910);
                ret = printf("package: %d\n", count++);
                time_wait(5);

                ret = fread(p,sizeof(char), flen-1810, plane);
                send_packet(p, flen-1810);
                ret = printf("package: %d\n", count++);
                time_wait(5);
            }*/
            fclose(plane);
