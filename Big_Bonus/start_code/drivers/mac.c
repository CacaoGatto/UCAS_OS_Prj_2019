...
void mac_irq_handle(void)
{
    ...
    mac_cnt++;
/*    if (valid_cnt < 32 && *((uint32_t*)(RDES_BASE_ADDR+4*PSIZE*num))!=0) {
        //memcpy((uint8_t*)pkg[valid_cnt], (uint8_t*)(RDES_BASE_ADDR+4*PSIZE*num), 4*PSIZE);
//        int i;
//        for (i = 0; i < PSIZE; i++) pkg[valid_cnt][i] = *((uint32_t*)(RDES_BASE_ADDR+4*PSIZE*num+4*i));
        valid_cnt++;
        //vt100_move_cursor(2, 4);
        //printk("> [MAC RECV TASK] start recv:(%d)                ",valid_cnt);
        //printk("> [MAC RECV TASK] start recv:(%d)%x",valid_cnt,pkg[valid_cnt][0]);
    }*/
...
uint32_t do_net_recv(uint32_t rd, uint32_t rd_phy, uint32_t daddr)
{
    ...
    */
    //int i;
    //for (i = 0; i < 32; i++) reg_write_32(DMA_BASE_ADDR + 0x8, 0x1);
    return 0;
}
