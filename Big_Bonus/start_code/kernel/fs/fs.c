...
void read_inode(uint16_t inode)
{
    ...
    memcpy((void*)&inode_buffer, (void*)(BUFFER + part * sizeof(inode_t)), sizeof(inode_t));
}
...
void write_inode(uint16_t inode)
{
    uint16_t sector = inode / 16, part = inode % 16;
    sdread((char*)BUFFER, IN_BASE_ADDR + sector * 512, 512);
    memcpy((void*)(BUFFER + part * sizeof(inode_t)), (void*)&inode_buffer, sizeof(inode_t));
...
void do_statfs()
{
    ...
    memcpy((void*)&superblock, (void*)BUFFER, sizeof(superblock_t));
...
int do_fwrite(int fd, char* buff, int size)
{
    ...
    for (i = start; i < end; i++) {
        int pos = pointer - i * BLOCK_SIZE;
        if (file->direct[i] == 0) file->direct[i] = alloc_block();
        read_block(file->direct[i]);
        int k;
        for (k = pos; k < BLOCK_SIZE; k++) {
            *((char*)(BUFFER + k)) = *buff++;
...
void do_loadelf()
{
    ...
    read_block(inode_buffer.direct[0]);
    
    Elf32_Ehdr* ehdr = (Elf32_Ehdr*)BUFFER;
    Elf32_Phdr* phdr = (Elf32_Phdr*)(BUFFER + ehdr->e_phoff);
    uint32_t p_filesz = phdr->p_filesz;
    uint32_t p_offset = phdr->p_offset;
    uint32_t e_entry = (uint32_t)ehdr->e_entry;
    
    uint32_t page = e_entry >> 23;
    uint32_t pte = (e_entry - (page << 23)) >> 13;
    pte_t* item;
    
    /* large elf
    uint32_t p_blocks = p_filesz / BLOCK_SIZE + 1;
    uint32_t p_pages = p_filesz / 0x2000 + 1;
    ...
    for (page = 0; page < MAX_PAGE; page++) {
        ...
        if (ref == 0x0000) ref = alloc_top_pte();
        pte = 0;
        while (pte + p_pages - count < MAX_PTE) {
            item = (pte_t*)(0xa0000000 + ref * 0x2000 + pte * 8);
            if (item->entrylo == 0) count++;
            else count = 0;
            if (count == p_pages) goto found_pte;
            pte++;
        }
    }
    vt100_move_cursor(1, ++pcb[0].line);
    printk("No available vaddr!");
    ...
    for (int i = 0; i < p_pages; i++) {
        ...
        item->entryhi = (page << 23) | (pte << 13) | (cur_pid & 0xff);
    ...
    */
    ...
    for (page = 0; page < MAX_PAGE; page++) {
        uint16_t ref = *((uint16_t*)(PTE_BASE_ADDR + page * 2));
        if (ref == 0x0000) ref = alloc_top_pte(page);
        for (pte = 0; pte < MAX_PTE; pte++) {
            item = (pte_t*)(0xa0000000 + ref * 0x2000 + pte * 8);
            if (item->entrylo == 0) goto found_pte;
        }
    }
    vt100_move_cursor(1, ++pcb[0].line);
    printk("No available vaddr!");
    */
    ...
    //found_pte:
    item->entryhi = (page << 23) | (pte << 13) | ((process_id+1) & 0xff);
    uint32_t pfn = (uint32_t)alloc_page();
    item->entrylo = (pfn << 7) | 0x16;
    
    set_entryhi(item->entryhi);
    set_entrylo(item->entrylo);
    uint32_t cp_index = get_index(item->entryhi);
    ...
    memcpy((void*)(0xa0000000 | pfn << 13), (void*)(BUFFER + p_offset), p_filesz);
    //memcpy((void*)(0xa0000000 | pfn << 13), (void*)(BUFFER), 0xc10);
    
    int idx = init_one_pcb();
    pcb[idx].user_context.cp0_epc = e_entry;
	pcb[idx].type = USER_PROCESS;
	pcb[idx].priority = 1;
    pcb[idx].pid = ++process_id;
    queue_push(&ready_queue,&pcb[idx]);
}
