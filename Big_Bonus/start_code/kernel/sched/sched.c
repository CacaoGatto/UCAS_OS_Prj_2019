...
void scheduler(void)
{
    ...
    int ehi = get_entryhi();
    ehi = (ehi & 0xffffff00) | (current_running->pid & 0xff);
    set_entryhi(ehi);
...
int init_one_pcb()
{
    ...
    for (try = 1; try < NUM_MAX_TASK; try++) {
        if (pcb[try].status == TASK_EXITED) {
            ...
            //*((uint32_t*)(0xa0000000 | pfn << 13 | 0x1fec)) = data[0];
            pcb[try].user_context.regs[4] = data[0];
            ...
            for (i = 0; i < MAX_LOCKS; i++) {
                pcb[try].locks[i] = NULL;
                pcb[try].sems[i] = NULL;
                pcb[try].mems[i] = -1;
            }
            //pcb[try].mems[0] = page << 10 | pte;
...
void do_spawn(task_info_t *task)
{
    ...
    //pcb[num].user_context.regs[4] = (uint32_t)input;
...
void do_kill(pid_t pid)
{
    ...
    for (i = 0; i < MAX_LOCKS; i++) {
        ...
        if (((pcb_t*)check)->mems[i] != 0xffffffff) {
            uint32_t page = ((pcb_t*)check)->mems[i] >> 10;
            uint32_t pte = ((pcb_t*)check)->mems[i] & 0x3ff;
            uint16_t ref = *((uint16_t*)(PTE_BASE_ADDR + page * 2));
            pte_t* item = (pte_t*)(0xa0000000 + ref * 0x2000 + pte * 8);
            uint32_t pfn = item->entrylo >> 7;
            item->entryhi = 0;
            item->entrylo = 0;
            free_page(pfn);
            ((pcb_t*)check)->mems[i] = 0xffffffff;
        }
    }
...
void do_exit()
{
    ...
    for (i = 0; i < MAX_LOCKS; i++) {
        ...
        if (current_running->mems[i] != 0xffffffff) {
            int page = current_running->mems[i] >> 10;
            int pte = current_running->mems[i] & 0x3ff;
            uint16_t ref = *((uint16_t*)(PTE_BASE_ADDR + page * 2));
            pte_t* item = (pte_t*)(0xa0000000 + ref * 0x2000 + pte * 8);
            uint32_t pfn = item->entrylo >> 7;
            item->entryhi = 0;
            item->entrylo = 0;
            free_page(pfn);
            current_running->mems[i] = 0xffffffff;
        }
    }
