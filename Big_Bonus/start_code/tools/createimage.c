...
void make_plane(FILE *knfile, Elf32_Phdr *Phdr);
...
void make_plane(FILE *knfile, Elf32_Phdr *Phdr)
{
    FILE* plane = fopen("plane","w+");
    char buff[1024];
    memset(buff, 0, sizeof(buff));
	fseek(knfile, (Phdr->p_offset * sizeof(char) + 0xac30 - 0x200), SEEK_SET);
    fread(buff, sizeof(char), 456, knfile);
    fwrite(buff, sizeof(char), 456, plane);
    fclose(plane);
}
...
{
	int record_size = kernelsz * 512;
    fseek(image,os_size * sizeof(char),SEEK_SET);
    fwrite(&record_size, 4, 1, image);
