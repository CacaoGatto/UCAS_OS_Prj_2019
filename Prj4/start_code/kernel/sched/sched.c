    ...
    for (try = 1; try < NUM_MAX_TASK; try++) {
        if (pcb[try].status == TASK_EXITED) {
            ...
            for (i = 0; i < MAX_LOCKS; i++) {
                ...
                pcb[try].sems[i] = NULL;
                pcb[try].mems[i].page = -1;
                pcb[try].mems[i].pte = -1;
            }
...
void do_spawn(task_info_t *task, unsigned long input[10])
{
...
void do_kill(pid_t pid)
{
    ...
    for (i = 0; i < MAX_LOCKS; i++) {
        ...
        if (((pcb_t*)check)->mems[i].page >= 0) {
            int page = ((pcb_t*)check)->mems[i].page;
            int pte = ((pcb_t*)check)->mems[i].pte;
            uint32_t ehi = pte_reference[page]->pte_list[pte]->entryhi | 0xffffff00;
            pte_reference[page]->pte_list[pte]->entryhi = ehi;
            ((pcb_t*)check)->mems[i].page = -1;
            ((pcb_t*)check)->mems[i].pte = -1;
        }
    }
...
void do_exit()
{
    ...
    for (i = 0; i < MAX_LOCKS; i++) {
        ...
        if (current_running->mems[i].page >= 0) {
            int page = current_running->mems[i].page;
            int pte = current_running->mems[i].pte;
            uint32_t ehi = pte_reference[page]->pte_list[pte]->entryhi | 0xffffff00;
            pte_reference[page]->pte_list[pte]->entryhi = ehi;
            current_running->mems[i].page = -1;
            current_running->mems[i].pte = -1;
        }
    }
