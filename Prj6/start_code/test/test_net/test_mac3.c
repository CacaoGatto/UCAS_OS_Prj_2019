...
static void mac_send_desc_init(mac_t *mac)
{
    ...
    for (i = 0; i < PNUM - 1; i++) {
        ...
        for (j = 0; j < PSIZE; j++) tbuf[i][j] = buffer[j];
    }
    ...
    for (j = 0; j < PSIZE; j++) tbuf[PNUM-1][j] = buffer[j];
...
static void mac_recv_desc_init(mac_t *mac)
{
    ...
    for (i = 0; i < PNUM - 1; i++) {
        ...
        rdes[i].des1 = 0x01000000 | (4 * PSIZE);
        rdes[i].des2 = (RDES_BASE_ADDR + i * 4 * PSIZE) & 0x1fffffff;
        rdes[i].des3 = ((uint32_t)&rdes[i+1]) & 0x1fffffff;
    }
    rdes[PNUM-1].des0 = 0x80000000;
    rdes[PNUM-1].des1 = 0x03000000 | (4 * PSIZE);
    rdes[PNUM-1].des2 = (RDES_BASE_ADDR + (PNUM-1) * 4 * PSIZE) & 0x1fffffff;
    rdes[PNUM-1].des3 = ((uint32_t)&rdes[0]) & 0x1fffffff;
    
    memset((void*)RDES_BASE_ADDR, 0, PNUM * 4 * PSIZE);
    
    mac->daddr = RDES_BASE_ADDR;
    mac->daddr_phy = RDES_BASE_ADDR & 0x1fffffff;
